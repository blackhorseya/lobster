...
func (i *impl) Create(ctx contextx.Contextx, created *okr.Goal) (*okr.Goal, error) {
	...
	cmd := `INSERT INTO goals (id, title, start_at, end_at, created_at) VALUES (:id, :title, :start_at, :end_at, :created_at)`
...
func (i *impl) QueryByID(ctx contextx.Contextx, id string) (*okr.Goal, error) {
	...
	var ret okr.Goal
	cmd := "SELECT id, title, start_at, end_at, created_at FROM goals WHERE id = ?"
...
func (i *impl) List(ctx contextx.Contextx, offset, limit int) ([]*okr.Goal, error) {
	...
	var ret []*okr.Goal
	cmd := "SELECT id, title, start_at, end_at, created_at FROM goals LIMIT ? OFFSET ?"
